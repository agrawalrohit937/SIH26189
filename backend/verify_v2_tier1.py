"""
V2 Tier 1 Acceptance Test Suite
SIH26189 - National Crime Intelligence Grid (Ministry of Home Affairs)

Uses FastAPI TestClient to directly invoke all API endpoints, database queries, and ML models.
Executes and verifies:
- 1.1 GAT-based link prediction with attention justification & edge removal acceptance test.
...
def test_1_1_gat_link_prediction(token):
    ...
    pred_data = resp.json()
    print("Initial Top GAT Predictions:")
    print(json.dumps(pred_data, indent=2))
    
    # 2. Edge removal test: Find a real relationship, remove it, re-run GAT, check if predicted
    ...
    if edge_sample:
        ...
        if found_recovered_edge:
            ...
            print("[INFO] Showing sample recovered predicted link structure:")
            if preds_after:
                print(json.dumps(preds_after[0], indent=2))
...
def test_1_2_burner_evasion_chain(token):
    ...
    # 1. Inject synthetic person with 5 phone numbers first-seen within 10 days
    ...
    # Create test person and burner numbers
    db.execute_query("""
    ...
    # 2. Call evasion alerts endpoint
    ...
    # Verify alert exists for test person
...
def test_1_3_active_learning_feedback(token):
    ...
    # 1. Fetch top link before feedback
    ...
    # 3. Fetch predictions after feedback
...
def test_1_4_document_rag_copilot(token):
    ...
    # Query detail that exists in narrative FIR text
    ...
    assert "FIR#" in reply or "per FIR" in reply or "CR-2026" in reply or "Cyber Crime" in reply, "Expected traceable FIR citation in response"
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("ALL TIER 1 V2 DIFFERENTIATORS VERIFIED SUCCESSFULLY")
    print("="*60)
