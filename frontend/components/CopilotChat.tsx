...
export const CopilotChat: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end pointer-events-none">
          ...
          <div className="flex items-center justify-between px-3.5 py-2.5 bg-slate-950 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400">Official Crime Analyst AI</p>
              </div>
            </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-3.5 space-y-3 text-xs">
                {mounted &&
                  messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        {isAi && (
                          <div className="w-6 h-6 rounded bg-slate-800 border border-slate-700 flex items-center justify-center text-amber-500 shrink-0 mt-0.5">
                ...
                {isThinking && (
                  <div className="flex items-start gap-2">
                    <div className="w-6 h-6 rounded bg-slate-800 border border-slate-700 flex items-center justify-center text-amber-500 shrink-0">
                      <Scale className="w-3 h-3" />
                    </div>
                    <div className="rounded-lg p-2.5 bg-slate-950 border border-slate-800 text-slate-300 flex items-center gap-2">
                      <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-500" />
                      <span className="text-[11px]">Analyzing legal intelligence...</span>
                    </div>
                  </div>
                )}
              ...
              <div className="p-2.5 bg-slate-950 border-t border-slate-800 flex items-center gap-1.5">
                <input
                  ...
                  disabled={isThinking}
                  className="flex-1 bg-slate-900 border border-slate-700 focus:border-amber-500 rounded-md px-2.5 py-1.5 text-xs text-slate-100 placeholder:text-slate-500 outline-none transition-colors disabled:opacity-60"
                />
                <button
                  onClick={() => handleSendMessage()}
                  disabled={!inputMessage.trim() || isThinking}
                  className="p-1.5 rounded-md bg-amber-600 hover:bg-amber-500 active:scale-95 disabled:opacity-40 disabled:pointer-events-none text-slate-950 font-semibold transition-all cursor-pointer shadow-sm"
                  title="Send (Enter)"
                >
      ...
      {/* Official Government FAB Button */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`pointer-events-auto relative w-12 h-12 rounded-full bg-slate-900 hover:bg-slate-800 text-amber-400 border border-slate-700 flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer ${
          isOpen ? "ring-2 ring-amber-500/60" : ""
        }`}
